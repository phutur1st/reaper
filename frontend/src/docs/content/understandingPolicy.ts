...
export const understandingPolicy: Doc = {
  ...
  body: [
    ...
    p(
      "**Caps stop a run; grace can hold a title.** [Pace and limits](understanding-policy#pace) sets both. Grace shows every flagged title as leaving for a set number of days. Enable Wait for grace before deleting to block deletion until that countdown ends. Review shows time remaining on movies and seasons, with exact dates in their details. Notice-only countdowns do not block deletion. You keep a title by watching it or sparing it. Nothing goes until you run a reap yourself.",
    ),
    p(
